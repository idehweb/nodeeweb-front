...
import { getProductRecoms } from '#c/functions/Order';
import PostCard from '../Home/PostCard';

// export const APP_VERSION = process.env.REACT_APP_VERSION_NUM;
const CardSidebar = ({ onNext, t }) => {
  ...
  useEffect(() => {
    (async () => {
      try {
        const recomProducts = await getProductRecoms(card);
        setRecoms(recomProducts);
      } catch (err) {}
    })();
  }, [card]);
      ...
      <CardHeader className={'pd-1'}>
        <div className="kjhghjk">
          <div
            ...
            dangerouslySetInnerHTML={{ __html: t('Cart') }}
          />
        </div>
      </CardHeader>
        ...
        {card && card.length > 0 ? (
          <Col lg="12">
            <div flush="true" className={'card-add'}>
              ...
              {recoms?.length ? (
                <div className={'the-swipppper-parent swiper-recom'}>
                  <Swiper
                    perPage={parseInt(3)}
                    autoplay
                    arrows
            ...
            <div className={'fdsdf pl-3 pr-3'} onClick={handleToggleCardbar}>
              {card && card.length > 0 && (
                <Button
                  onClick={() => {
                    const card = store.getState().store.card;
                    console.log(' card', card);
                    if (!card || (card && !card[0])) {
                  ...
                  <span className={'juytrftyu'}>
                    {tsum && (
                      <span className={'ttl gtrf'}>{returnPrice(tsum)}</span>
                    )}
                  </span>
                </Button>
              )}
