...
import {ApiUrl} from "#c/functions"
...
const ChatCloseIcon = () =>{
 ...
 const ChatIcon = (props) =>{
    ...
    if(action)
    return (
        <div id="box-widget-icon" className="fadeInUp" onClick={()=>onToggle(true)}>
...
const ChatContent =   (props) =>{
    ...
    const Header = styled(Paper)(({ theme }) => ({
      ...
      const handlerCallBackMessage = async (value) =>{
        ...
        if(localStorage.getItem('onlineSupport')){
             tokenID = localStorage.getItem('onlineSupport');
            if(value){
                ...
                AllMessages.push(messageObjClient)
                setMessages(messages=>[...messages,messageObjClient])
                Block.dots('.chat');
                await axios.post(ApiUrl+'/gateway/chatgpt/chat', messageObjClient,{headers}).then((res)=>{
                    if(res){
                        const {data} = res;
                         let messageObjSupport = {
                            ...
                            username:data.model,
                            type:'support',
                            text:data && data.choices && data.choices[0].message.content
                        }
                        AllMessages.push(messageObjSupport)
      ...
      React.useEffect(()=>{
        if(localStorage.getItem('onlineSupport')){
            const cookieID = localStorage.getItem('onlineSupport');
            const cookieChat = Cookies.get(cookieID);
            setMessages(messages => [...messages,cookieChat] );
            AllMessages = cookieChat;
            console.log('CookieChatIs', AllMessages);
        }else{
            const cID = makeid(50);
            localStorage.setItem('onlineSupport',cID);
            AllMessages = [];
        }
      },[])
    if(action)
                            ...
                            <Grid container spacing={1} direction="column">
                                ...
                                <Header>
                                    <Grid container spacing={2} columns={12}>
                                        <Grid item xs={3}>
                                            <span className={'chatAvatar'} title={title} data-container="body" rel="tooltip" data-placement="auto left" data-original-title={title}>
                                                <img src="https://cdn.goftino.com/static/assets/img/profile.png" alt="avatar" width="40" height="40" />
                                            </span>
                                        </Grid>
                                        <Grid item xs={6}>
 ...
 const ChatBase = (props) =>{
    ...
    const {position,left,bottom,width,top,height,active} = fields;
    if(active)
    return(
        <>
            <section style={{
            ...
            <ChatIcon action={!toggleChat} onToggle={(e)=>setToggleChat(e)}/>
            <ChatContent options={fields} action={toggleChat} onClose={(e)=>setToggleChat(false)}/>
            </section>
        </>
    )
}
