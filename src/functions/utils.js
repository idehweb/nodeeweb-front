...
export const postAdminData = (url = "", data = {}, f = false, headers = {}, rewriteHeader = false) => {
  ...
  return new Promise(function (resolve, reject) {
    ...
    if (isClient)
      if (f) {
        ...
        // if (token) {
        // console.log(store.getState().store);
        option["headers"]["token"] = store.getState().store.admin.admin_token || '';
        // }
        // else {
        //   console.log("postData redirect login");
        // return
...
export const putAdminData = (url = "", data = {}, f = false) => {
  return new Promise(function (resolve, reject) {
    ...
    if (isClient)
      if (f) option["headers"]["token"] = store.getState().store.admin.admin_token || '';
      else {
        console.log("postData");
        return;
        window.location.replace("/login");
